		...
		expected := []query.Component{
			{
				...
				ProductFilter: &product.Filter{
					...
					AttributeFilters: []*product.AttributeFilter{
						{Key: "Instance Type", Value: util.StringPtr("db.t2.xlarge")},
						{Key: "Deployment Option", Value: util.StringPtr("Single-AZ")},
						{Key: "Database Engine", Value: util.StringPtr("PostgreSQL")},
					},
				},
			...
			},
			{
				...
				ProductFilter: &product.Filter{
					...
					AttributeFilters: []*product.AttributeFilter{
						{Key: "Deployment Option", Value: util.StringPtr("Single-AZ")},
						{Key: "Volume Type", Value: util.StringPtr("General Purpose")},
					},
				},
			},
		}
		...
		expected := []query.Component{
			...
			},
			{
				...
				ProductFilter: &product.Filter{
					...
					AttributeFilters: []*product.AttributeFilter{
						{Key: "Deployment Option", Value: util.StringPtr("Single-AZ")},
						{Key: "Volume Type", Value: util.StringPtr("Provisioned IOPS")},
					},
				},
			},
			{
		...
		expected := []query.Component{
			{
				...
				ProductFilter: &product.Filter{
					...
					AttributeFilters: []*product.AttributeFilter{
						{Key: "Instance Type", Value: util.StringPtr("db.t2.xlarge")},
						{Key: "Deployment Option", Value: util.StringPtr("Multi-AZ")},
						{Key: "Database Engine", Value: util.StringPtr("Oracle")},
						{Key: "Database Edition", Value: util.StringPtr("Standard One")},
						{Key: "License Model", Value: util.StringPtr("Bring your own license")},
					},
				},
				PriceFilter: &price.Filter{
					Unit: util.StringPtr("Hrs"),
