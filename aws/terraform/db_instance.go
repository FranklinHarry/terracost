...
func (inst *DBInstance) databaseInstanceComponent() query.Component {
	...
	attrFilters := []*product.AttributeFilter{
		...
		{Key: "Deployment Option", Value: util.StringPtr(inst.deploymentOption)},
		{Key: "Database Engine", Value: util.StringPtr(inst.databaseEngine)},
	}

	if inst.databaseEdition != "" {
		f := &product.AttributeFilter{Key: "Database Edition", Value: util.StringPtr(inst.databaseEdition)}
		attrFilters = append(attrFilters, f)
	}

	if inst.licenseModel != "" {
		f := &product.AttributeFilter{Key: "License Model", Value: util.StringPtr(inst.licenseModel)}
		attrFilters = append(attrFilters, f)
	}
	...
	return query.Component{
		...
		ProductFilter: &product.Filter{
			...
			AttributeFilters: []*product.AttributeFilter{
				{Key: "Deployment Option", Value: util.StringPtr(inst.deploymentOption)},
				{Key: "Volume Type", Value: util.StringPtr(volumeType)},
			},
		},
	}
}
